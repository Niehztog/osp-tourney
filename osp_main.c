...
int OSP_setID(edict_t *ent)
{
    ...
    for (i = 1; i <= game.maxclients; i++) {
        ...
        if (d > bestdp && d > 0.9f && PlayerIdCanSee(ent, cl)) {
            bestdp = d;
            best_ent = cl;
        }
    }
    ...
    if (bestdp > 0.9f) {
        if (m_mode == 2 && (ent->client->resp.team == 2 ||
            ...
            Q_snprintf(line, sizeof(line), "Teammate \"%s\"\n",
                       best_ent->client->pers.greenname);

            if (strcmp(ent->client->resp.osp_r038, line)) {
                Q_strlcpy(ent->client->resp.osp_r038, line,
            ...
            if (ent->client->resp.osp_r204) {
                Q_snprintf(line, sizeof(line), "Viewing \"%s\"",
                           best_ent->client->pers.netname);
