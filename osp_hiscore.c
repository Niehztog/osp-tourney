...
{
    char    pname[OSP_HS_FIELD];
    char    sc[OSP_HS_FIELD];
    char    date[OSP_HS_FIELD];
    char    fn[MAX_OSPATH];
    char    ddir[MAX_OSPATH];
    int     j;
    FILE    *fin = NULL;
    cvar_t  *gamedir;
    cvar_t  *base;
    ...
    if (gamedir && base) {
        Q_snprintf(ddir, sizeof(ddir), "%s/%s", base->string,
                   gamedir->string);
        if (Q_snprintf(fn, sizeof(fn), "%s/%s/%d/%s", ddir, hsdir->string,
                       (int)port->value, level.mapname) >= sizeof(fn)) {
        ...
        if (fin) {
            ...
            if ((hs_mode == 1 && (strcmp(pname, "FL") || hs_limit != Q_atoi(sc))) ||
                (hs_mode == 2 && (strcmp(pname, "TL") || hs_limit != Q_atoi(sc)))) {
            ...
            for (j = 0; j < 10; j++) {
                ...
                Q_strlcpy(p_table[j].name, pname, sizeof(p_table[j].name));
                Q_strlcpy(p_table[j].score, sc, sizeof(p_table[j].score));
                Q_strlcpy(p_table[j].date, date, sizeof(p_table[j].date));
            }
...
void OSP_writeHighScores(void)
{
    ...
    if (gdir && base) {
        Q_snprintf(path, sizeof(path), "%s/%s", base->string,
                   gdir->string);
        if (Q_snprintf(fn, sizeof(fn), "%s/%s/%d/%s", path, hdir->string,
                       (int)port->value, level.mapname) >= sizeof(fn)) {
            gi.dprintf("High score path too long.\n");
            return;
        }
