...
/*
 * Monster spawn stubs.  OSP Tourney is pure PvP, so every monster entry point
 * the vanilla spawn table can reach is reduced to freeing the entity; spawns[]
 * still maps every monster_* classname here, so a map with monsters loads and
 * simply has none.  They open THIS object: real's ELF has an `as` nop
 * (8d 76 00) between SP_misc_insane and Use_Areaportal, not ld padding, and
 * real's DLL starts Use_Areaportal at an unaligned 100174D9 with no int3 --
 * neither image has an object boundary there.
 */
