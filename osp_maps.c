    ...
    if (map_size && !selected_map) {
        ...
        do {
            if (map_random && (int)map_random->value) {
                // v2.75 seeded here with ftime() -- the whole seconds on Linux,
                // the milliseconds on Windows.  The draw below is Q_rand(),
                // which srand() does not feed, so the reseed did nothing;
                // Q_srand() is the one that does.
                Q_srand(time(NULL));
