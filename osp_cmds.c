...
void OSP_vote_cmd(edict_t *ent, int mode, int nargs, char *what, char *value)
{
    ...
    if (!Q_stricmp(arg1, "map")) {
        ...
        if (nc == 2)
            gi.cprintf(ent, PRINT_HIGH, "Current timelimit: %d\n",
                       (int)timelimit->value);
        else if (Q_atoi(arg2) < 0 || Q_atoi(arg2) > (int)menu_maxtime->value)
        ...
        if (nc == 2)
            gi.cprintf(ent, PRINT_HIGH, "Current fraglimit: %d\n",
                       (int)fraglimit->value);
        else if (Q_atoi(arg2) < 0 || Q_atoi(arg2) > (int)menu_maxfrag->value)
        ...
        if (nc == 2)
            gi.cprintf(ent, PRINT_HIGH, "Current item toggles: %d\n",
                       item_settings);
        else if (Q_atoi(arg2) < 0 || Q_atoi(arg2) >= 256)
            gi.cprintf(ent, PRINT_HIGH,
                       "Invalid item toggle setting (%d) (%d)!\n",
                       Q_atoi(arg2), item_settings);
        else
            vote_item = 0x20;
        ...
        if (nc == 2)
            ...
            if (!kick) {
                ...
                // local: arg2 points into the engine's tokenizer buffer, and a
                // short token there has no room for a three-digit id.
                Q_snprintf(kickid, sizeof(kickid), "%d",
                           kick->client->resp.clientid);
                arg2 = kickid;
                vote_item = 0x1000;
            }
        }
        ...
        else if (Q_atoi(arg2) < 0 || Q_atoi(arg2) >= nbots)
        ...
        if (nc == 2)
            ...
            gi.cprintf(ent, PRINT_HIGH,
                 ...
                 bots_votedin == (int)vote_bots_max->value)
            gi.cprintf(ent, PRINT_HIGH, "Sorry, cannot add anymore bots.\n");
        else if (Q_atoi(arg2) + connected_clients > (int)game.maxclients)
            gi.cprintf(ent, PRINT_HIGH, "You can add in only %d more bots.\n",
                       (int)game.maxclients - connected_clients);
        else if (Q_atoi(arg2) + bots_votedin > (int)vote_bots_max->value)
            gi.cprintf(ent, PRINT_HIGH, "You can add in only %d more bots.\n",
        ...
        else if (Q_atoi(arg2) > bots_votedin)
    ...
    if (vote_item) {
        ...
        Q_strlcpy(vote_value, arg2 ? arg2 : "", sizeof(vote_value));
        vote_yea = 1;
        ...
        gi.bprintf(PRINT_HIGH, "%s has initiated a vote!\n",
                   ent->client->pers.greenname);
        OSP_voteinfo(ent, 1);
        OSP_Stats_Vote("Propose", arg1, arg2);
        OSP_checkVote();
    }
}
