...
void OSP_setMOTD(void)
{
    ...
    if (gamedir && base) {
        ...
        char    *pp = path;
        int     ch = -1;    // block scope: real's PE puts it below path and pp

        Q_snprintf(path, sizeof(path), "%s/%s/%s", base->string,
                   gamedir->string,
                   motdname ? motdname->string : "motd.txt");
    ...
    if (!(int)center->value) {
        ...
        for (j = 0; j < lines; j++, y += 8) {
            Q_snprintf(buf, sizeof(buf), "yb %d string \"%s\"", y, pagebuf[j]);
        ...
        for (j = 0; j < lines; j++, y += 8) {
            Q_snprintf(buf, sizeof(buf), "yv %d string \"%s\"", y, pagebuf[j]);
            Q_strlcat(match_motd, buf, sizeof(match_motd));
        }
...
void OSP_showPlayer(edict_t *ent)
{
    ...
    Q_snprintf(name, sizeof(name), "Player: %s (%s)", pl->client->pers.greenname, pl->client->resp.osp_r0f4);
    Q_snprintf(buf, sizeof(buf), "xv 0 yv 0 string2 \"%s\"", name);

    Q_strlcpy(txt, "_", sizeof(txt));
    {
        for (cidx = 0; cidx < strlen(name) - 1 && cidx < 59; cidx++)
            Q_strlcat(txt, "_", sizeof(txt));
    }

    Q_snprintf(name, sizeof(name), "yv 4 string2 \"%s\"", txt);
    ...
    if (frags < 1 || (!dth && !frags))
        ...
        eff = 100.0f * frags / (0.0f + frags + dth);

    Q_snprintf(txt, sizeof(txt), "yv %d string \"Frags   :%3d     Efficiency: %.1f%%\"",
            y, frags, eff);
    Q_strlcat(buf, txt, sizeof(buf));
    y += 8;
    ...
    Q_snprintf(txt, sizeof(txt), "yv %d string \"Deaths  :%3d     Frags/Hour: %d\"",
            y, dth, fph);
    Q_strlcat(buf, txt, sizeof(buf));
    y += 8;

    Q_snprintf(txt, sizeof(txt), "yv %d string \"Suicides: %2d     Rank: %d/%d\"",
            y, selfkills, pl->client->resp.osp_r208, active_clients);
    Q_strlcat(buf, txt, sizeof(buf));
    y += 16;

    cidx = pl->client->resp.clientid;
    if (cidx < 0 || cidx >= q_countof(p_acc)) {
    ...
    for (k = 0; k < 10; k++) {
        ...
        if (p_acc[cidx].shots[aidx]) {
            Q_snprintf(txt, sizeof(txt), "yv %d string \"%s %.1f%% (%d/%d hits)\"", y,
                    a_info[k].name,
    ...
    if (!any) {
        Q_snprintf(txt, sizeof(txt), "yv %d string \"Hasn't taken a shot.\"", y);
        Q_strlcat(buf, txt, sizeof(buf));
        y += 8;
    } else {
        y += 8;
        Q_snprintf(txt, sizeof(txt), "yv %d string2 \"Total damage given: %d\"", y,
                p_acc[cidx].dgiven);
        Q_strlcat(buf, txt, sizeof(buf));
        y += 8;
        Q_snprintf(txt, sizeof(txt), "yv %d string2 \"Total damage rcvd : %d\"", y,
                p_acc[cidx].dtaken);
        Q_strlcat(buf, txt, sizeof(buf));
        y += 8;
    }
    ...
    y += 8;
    Q_snprintf(txt, sizeof(txt), "yv %d cstring \"\x90 CONTINUE \x91\"", y);
    Q_strlcat(buf, txt, sizeof(buf));
